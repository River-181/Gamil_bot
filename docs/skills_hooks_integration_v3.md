...
## 9) 새 작업 타입 연동: legacy-label-archive-migration
- Phase 9 런북(`docs/runbooks/legacy_label_archive_migration_v1.md`)에서 출력하는 실행 이벤트:
  - `archive_migrate` 완료 시:
    - `run_id`
    - `stage`
    - `messages_mutated`
    - `rollback_ready`
  - `archive_rollback` 완료 시:
    - `run_id`
    - `rolled_back`
    - `remaining_impacted_messages`
- 출력 예시:
  - `{"status":"ok","run_id":"...","stage":"A","messages_mutated":200,"rollback_ready":true}`
- PostToolUse는 민감정보 마스킹을 유지하고 checkpoint/journal 경로만 기록
