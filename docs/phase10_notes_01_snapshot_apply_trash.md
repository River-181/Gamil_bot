...
- `TrashCandidate`는 즉시 삭제하지 않고 보존 후 `TRASH`로 이동한다.
...
## 추천 실행 순서
1. `--build-snapshot --snapshot-limit 50`
2. snapshot 샘플 리뷰
3. `--apply-snapshot <file>`
4. 결과/저널 확인
5. 규모 확대: `50 -> 200 -> 500 -> 1000`
6. `@AUTO/TrashCandidate older_than:7d`부터 `--trash-commit`
...
- Gmail metadata 조회가 느리므로, bulk 규칙군만 따로 snapshot하는 방식이 더 안정적이다.
